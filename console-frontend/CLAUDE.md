...
When `NewTemplate` includes a file, the form submits multipart to `/api/templates/upload`; otherwise it stays on the JSON path. `TemplateDetail` shows a small chip with the document filename + slide count when one was attached, and its "Start meeting" button saves any pending edits then navigates to `/meetings/new?template=<id>` — there is no start-meeting modal. `NewMeeting` creates a single meeting: pick a ready template (preselected from the `?template=` deep link), set a title + duration (both above, mode-independent), then act in one of two stacked subheading sections shown at once — no mode toggle. The **Start now** section's Start button calls `POST /api/templates/:id/meetings` and shows a result panel with copyable join + live-view links (`CopyButton`). The **Schedule for later** section has a calendar + time picker + an invitees field (comma/newline emails → `string[]`) and its Schedule button calls `POST /api/templates/:id/scheduled-meetings`; its result panel offers **Add to calendar (.ics)** (a download `<a>` to `meetingInviteIcsUrl(id)`), a copyable stable live-view link, and the invited list. The in-flight `busy` state names which section is submitting so only that button disables/spins; an error renders inside the section that triggered it. It is reachable from the dashboard **New meeting** button and from any template's Start button. `NewBatchMeetings` (`/meetings/batch`, the dashboard's **Users** button) reuses the single page's template picker + title/duration above, then the same **Schedule** section (the shared `Calendar` month grid + a time picker, identical to `NewMeeting`'s "Schedule for later"). It is **schedule-only** — there is no Start now path and no mode toggle (batches are meant to be scheduled). Instead of one title/invitees it carries a repeatable **interviewee** list — one `[Name] [Email]` row per person (`ListPlus` to add, `X` to remove; the last row isn't removable), the name becoming each meeting's title and the email its invitee. **Schedule** → `POST /api/templates/:id/scheduled-batch-meetings`, launching one parallel scheduled meeting per row; a combined result panel lists every created meeting (with per-row `.ics` / copy-link buttons) above a destructive `Alert` enumerating any per-row failures. (`startBatchFromTemplate` / `…/batch-meetings` still exists in the API for the start-now path, just no longer surfaced in this page.)
...
| --- | --- | --- |
| Primary / commit | `default` (filled) | New, Create, Start, Schedule, Retry generation, Go to meeting, Add to calendar |

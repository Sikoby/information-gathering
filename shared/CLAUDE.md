...
- `src/components/ui/*` — shadcn (new-york) primitives: `alert`, `badge`, `button`, `calendar`, `card`, `collapsible`, `dropdown-menu`, `input`, `modal`, `progress`, `separator`, `textarea`, `tooltip`; plus generic app-shell layout bars `header` + `footer`. `modal` is a dependency-free centered dialog (portal + overlay, closes on Escape / overlay click / corner button, locks body scroll) — callers pass `title`, `description`, optional `children` body and `footer` actions; back error popups, confirmations, etc. (a styled `<header>`/`<footer>` with a centered `max-w-6xl` container — apps pass the brand/actions/footer content as children). `tooltip` also exports `InfoTooltip` — a self-contained info-`(i)`-icon-with-tooltip (embeds its own `TooltipProvider`, so it is drop-in with no app-root wiring). Its `size` prop (`sm`/`md`/`lg`, default `md`) scales the icon to the heading it sits beside. Depends on `@radix-ui/react-tooltip` and `lucide-react`. `calendar` is the shadcn month-grid wrapper around `react-day-picker` (which bundles `date-fns`) and reuses `buttonVariants` from `button`.

...
| `meeting:<meeting_id>` | string (JSON) | console | console | a thin meeting instance referencing a template_id, plus `owner_email`; `status` is `scheduled`\|`running`\|`done`, with `scheduled_at`+`invitees` on a scheduled meeting (`invitees` embedded as a JSON *string* to survive the Lua merge); no TTL, AOF-persisted |

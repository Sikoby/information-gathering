...
│  │  - phase │  │ Follow-ups                           │  │
│  │  - phase │  │ ─── Separator ───                    │  │
│  │ Followup │  │ Briefing (collapsible markdown)      │  │
│  │ Briefing │  │                                      │  │
│  └──────────┘  └──────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────┘
```
...
The agent owns a single tree of `Section` nodes (`kind` ∈ `meeting | topic | question | answer`). The root is `kind="meeting"` (id `_root`). Its `header`+`body` are seeded at startup from the template's `name` + `description` (the agent starts the cursor in the first phase — the introduction — so there is no separate `frame_meeting` step). The root node itself is not rendered in the viewer; the title comes from `template.name`. Top-level TOPICs with a `target_fraction` set are "phases" — together they form the agenda; their fractions must sum to ~1.0. Inside a phase TOPIC sit other TOPICs and QUESTIONs; QUESTIONs own only ANSWERs, which are runtime nodes (`record_finding` writes them). The pyramid wrap-up is just another TOPIC under root with the well-known id `_root/closing`. The viewer derives the current phase from `enclosingPhase(state.sections, state.current_section_id)` — never from a stored field.
...
- Anchor `id`s are stable: `breadcrumb`, `agenda`, `notebook`, `section-<sectionId>` (per node in the tree), `followups`, `briefing`. The Sidebar reads these. Don't rename without updating `Sidebar.tsx`.
- `scroll-mt-24` keeps headings clear of the fixed header on smooth-scroll.

`Card`-style framing (`rounded-md border bg-card p-3`) is reserved for items — a single answer, a single question.
...
| --- | --- | --- |
| `Badge` | Header, Agenda, Notebook | Pills (phase, status, kind, target fraction) |
